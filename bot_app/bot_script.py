...
from aiogram.types import LabeledPrice, ContentType, InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo, MenuButtonWebApp
...
# !!! RAILWAY LINKINI SHU YERGA YOZING !!!
WEBAPP_URL = "https://sizning-saytingiz.railway.app/shop/" 
...
def get_product_kb(product_id, quantity=1):
    kb = InlineKeyboardMarkup(row_width=3)
    kb.row(
        InlineKeyboardButton("➖", callback_data=f"minus_{product_id}_{quantity}"),
        InlineKeyboardButton(f"{quantity}", callback_data="ignore"),
        InlineKeyboardButton("➕", callback_data=f"plus_{product_id}_{quantity}")
    )
    kb.add(InlineKeyboardButton("🛒 Savatchaga qo'shish", callback_data=f"addcart_{product_id}_{quantity}"))
    return kb
...
async def web_app_data_handler(message: types.Message):
    p_id = message.web_app_data.data
...
async def cb_cat(c: types.CallbackQuery):
    ...
    for p in prods:
        await bot.send_message(c.from_user.id, f"📦 {p.name}\n💰 {p.price:,} so'm", reply_markup=get_product_kb(p.id))

@dp.callback_query_handler(lambda c: c.data.startswith(('plus_', 'minus_')), state="*")
async def update_quantity(c: types.CallbackQuery):
    action, p_id, qty = c.data.split('_')
    qty = int(qty)
    if action == 'plus': qty += 1
    elif action == 'minus' and qty > 1: qty -= 1
    else: return await c.answer("Minimal 1 ta tanlash mumkin")
    await c.message.edit_reply_markup(reply_markup=get_product_kb(p_id, qty))
    await c.answer()

@dp.callback_query_handler(lambda c: c.data.startswith('addcart_'), state="*")
async def cb_add_cart(c: types.CallbackQuery):
    _, p_id, qty = c.data.split('_')
    user = await sync_to_async(TelegramUser.objects.get)(user_id=c.from_user.id)
    product = await sync_to_async(Product.objects.get)(id=p_id)
    for _ in range(int(qty)):
        await sync_to_async(Cart.objects.create)(user=user, product=product)
    await bot.answer_callback_query(c.id, text=f"{qty} ta savatchaga qo'shildi! ✅")
...
async def view_cart(message: types.Message, state: FSMContext):
    ...
    summary = {}
    for i in items: summary[i.product.name] = summary.get(i.product.name, 0) + 1
    for name, count in summary.items(): text += f"🔹 {name} x {count} ta\n"
    
    text += f"\n💰 Jami: <code>{total_price:,}</code> so'm"
    if discount > 0: text += f"\n🎁 Promokod: <code>-{discount}%</code>\n✅ To'lov: <code>{final_price:,}</code> so'm"
    
    kb = InlineKeyboardMarkup(row_width=1).add(
        InlineKeyboardButton("💳 Sotib olish", callback_data="checkout_cart"),
        InlineKeyboardButton("🎟 Promokod", callback_data="apply_promo"),
...
async def promo_check(message: types.Message, state: FSMContext):
    promo = await sync_to_async(PromoCode.objects.filter(code=message.text.strip(), is_active=True).first)()
    if promo:
        await state.update_data(discount=promo.discount_percent)
        await message.answer(f"✅ Chegirma: {promo.discount_percent}%")
...
async def cart_checkout(c: types.CallbackQuery, state: FSMContext):
    user = await sync_to_async(TelegramUser.objects.get)(user_id=c.from_user.id)
    items = await sync_to_async(list)(Cart.objects.filter(user=user).select_related('product'))
    if not items: return await c.answer("Savat bo'sh!")
    
    data = await state.get_data()
    final = sum(i.product.price for i in items) * (100 - data.get('discount', 0)) // 100
    await bot.send_invoice(c.from_user.id, "🛍 To'lov", "Xaridlar", "payload", PAYMENT_TOKEN, "UZS", [LabeledPrice("Jami", int(final*100))])
    await c.answer()
...
async def admin_users(message: types.Message, state: FSMContext):
    await state.finish(); await show_users_page(message, 1)
...
async def cb_upage(c: types.CallbackQuery):
    await show_users_page(c.message, int(c.data.split('_')[1])); await c.answer()
...
async def stats_view(message: types.Message, state: FSMContext):
    u = await sync_to_async(TelegramUser.objects.count)()
    o = await sync_to_async(Order.objects.filter(is_paid=True).count)()
...
async def ad_start(message: types.Message, state: FSMContext):
    await state.finish(); await ShopStates.waiting_ad.set()
...
async def ad_send(message: types.Message, state: FSMContext):
    users = await sync_to_async(list)(TelegramUser.objects.all()); count = 0
    for u in users:
        try: await message.copy_to(u.user_id); count += 1
        except: pass
    await state.finish(); await message.answer(f"✅ {count} kishiga yuborildi.", reply_markup=get_admin_kb())

# --- QIDIRUV, PROFIL, YORDAM ---
@dp.message_handler(lambda m: m.text == "🔍 Qidiruv", state="*")
async def search_start(message: types.Message, state: FSMContext):
    await state.finish(); await ShopStates.search.set()
...
async def search_process(message: types.Message, state: FSMContext):
    prods = await sync_to_async(list)(Product.objects.filter(name__icontains=message.text, is_sold=False)[:10])
    await state.finish()
    if not prods: await message.answer("😔 Topilmadi.", reply_markup=get_main_kb(message.from_user.id))
    else:
        for p in prods: await message.answer(f"📦 {p.name}\n💰 {p.price:,}", reply_markup=get_product_kb(p.id))
        await message.answer("Natijalar.", reply_markup=get_main_kb(message.from_user.id))

@dp.message_handler(lambda m: m.text == "👤 Profil", state="*")
async def profile_view(message: types.Message, state: FSMContext):
    user = await sync_to_async(TelegramUser.objects.get)(user_id=message.from_user.id)
    await message.answer(f"👤 Profil\nID: {user.user_id}\nBonus: {user.balance:,} so'm")

@dp.message_handler(lambda m: m.text == "ℹ️ Yordam", state="*")
async def help_h(message: types.Message): await message.answer("🆘 Admin: @az1z_0518")

@dp.message_handler(lambda m: m.text == "⬅️ Asosiy menyu", state="*")
async def back_main(message: types.Message, state: FSMContext):
    await state.finish(); await message.answer("🏠 Asosiy menyu", reply_markup=get_main_kb(message.from_user.id))
...
async def pay_success(message: types.Message, state: FSMContext):
    user = await sync_to_async(TelegramUser.objects.get)(user_id=message.from_user.id)
    items = await sync_to_async(list)(Cart.objects.filter(user=user))
    for item in items:
        await sync_to_async(Order.objects.create)(user=user, product=item.product, amount=item.product.price, is_paid=True)
        item.product.is_sold = True; await sync_to_async(item.product.save)()
    await sync_to_async(Cart.objects.filter(user=user).delete)()
    await message.answer("✅ To'lov bajarildi!", reply_markup=get_main_kb(message.from_user.id))

@dp.callback_query_handler(lambda c: c.data == "clear_cart")
async def cb_clear(c: types.CallbackQuery):
    user = await sync_to_async(TelegramUser.objects.get)(user_id=c.from_user.id)
    await sync_to_async(Cart.objects.filter(user=user).delete)()
    await bot.send_message(c.from_user.id, "🗑 Savatcha tozalandi."); await c.answer()
...
# bot_script.py faylida
WEBAPP_URL = "https://my-bot-production.up.railway.app/shop/" # Oxirida /shop/ bo'lishi SHART